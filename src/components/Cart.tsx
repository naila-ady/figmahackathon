...
const ShoppingCartComponent = () => {
  const ref = useRef<HTMLDivElement>(null); // move ref outside of toggleCart function
  ...
  return (
    <div>
      ...
      <div ref={ref} className='sidecart absolute bg-[#ffffff] w-64 top-0 right-0 py-10 px-6 transform transition-transform translate-x-full'>
        <h2 className='font-bold text-xl text-center text-[#ff9f0d]'>SHOPPING CART</h2>
        <span onClick={toggleCart} className='absolute top-4 right-4 text-[#ff9f0d] cursor-pointer text-2xl'>
          <CircleX />
        </span>
          ...
          {[...Array(7)].map((_, index) => (
            <li key={index}>
              <p className='item flex my-5'>
                <span className='w-2/3 text-xl'>DESERTS</span>
                <span className='w-1/3 text-2xl flex items-center justify-center'>
                  <CircleMinus className='cursor-pointer text-[#ff9f0d]' />
                  <span className='text-xl mx-3'>1</span>
                  <CirclePlus className='cursor-pointer text-[#ff9f0d]' />
                </span>
              </p>
            </li>
          ))}
